...
def jsd(cp: Tensor, cq: Tensor) -> Tensor:
    ...
    # TODO
    #m = 0.5 * (cp + cq)
    #return 0.5 * (torch.distributions.kl.kl_divergence(cp, m) +  torch.distributions.kl.kl_divergence(cq, m))
    #################################
    #cp = cp.to('cpu').numpy()
    #cq =cq.to('cpu').numpy()
    #print(cp)
    ...
    #return distance.jensenshannon(cp,cq)
    #p = np.asarray(cp)
    #q = np.asarray(cq)
    #p = p / np.sum(p, axis=0)
    #q = q / np.sum(q, axis=0)
    ...
    #ratio1 = cp / (cq + cp)
    #ratio0 = cq / (cq + cp)
    #expected = torch.sum( torch.log(ratio1)) + torch.sum((1-torch.log(ratio1)))
    #jsd = (expected + torch.log(torch.tensor(4))) / 2
    #return jsd
    cpp = torch.sigmoid(cp)
    cqq = torch.sigmoid(-cq)
    return ((torch.log(cpp)).mean()+(torch.log(cqq)).mean()+torch.log(torch.tensor(4))) /2
...
def w1(cp: Tensor, cq: Tensor) -> Tensor:
    ...
    """
    # TODO
    cp_mean = torch.mean(cp)
    cq_mean = torch.mean(cq)
    return cp_mean - cq_mean
    #raise NotImplementedError("Implement as part of assignment 3.")
...
def r1gp(p: Tensor, cp: Tensor,
    ...
    cpp = cp
    grad = torch.autograd.grad(cpp, p,
            grad_outputs=torch.ones_like(cpp), retain_graph=True,allow_unused=True,
            create_graph=True)[0].view(cpp.size(0),-1)

    return (torch.mean(grad.norm(dim=1,p=2)**2))
    #raise NotImplementedError("Implement as part of assignment 3.")
...
def ogp(p: Tensor, cp: Tensor,
    ...
    grad = torch.autograd.grad(y, x,
            grad_outputs=torch.ones_like(y), retain_graph=True,
            create_graph=True, only_inputs=True)[0].view(y.size(0),-1)
...
def apply_exponential_moving_average(source: nn.Module, target: nn.Module,
    ...
    with torch.no_grad():
        # TODO
        targets = [p for p in target.parameters()]
        sources = [p for p in source.parameters()]
        for t, s in zip(targets,sources):
             t.sub_((1.0 - alpha) * (t - s))
        #raise NotImplementedError("Implement as part of assignment 3.")
...
def make_train_step(loss_type: Text,
    ...
    def train_step(train_data_iter: Iterator[Tensor],
        ...
        generator_loss = metric(true_X, fake_X, critic)
        generator_loss.backward()
        generator_optim.step()
        
        if alpha_ema is not None:
            apply_exponential_moving_average(generator, test_generator, alpha=alpha_ema)
