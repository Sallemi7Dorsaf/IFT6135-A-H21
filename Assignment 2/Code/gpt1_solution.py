...
class LayerNorm(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # ==========================
        numerator = inputs - inputs.mean(dim=-1, keepdim=True)
        denominator = (((inputs - inputs.mean(dim=-1, keepdim=True)) ** 2).mean(dim=-1, keepdim=True) + self.eps).sqrt()
        layernorm = (numerator/denominator)*self.weight+self.bias
        return layernorm
...
class MultiHeadedAttention(nn.Module):
    def __init__(self, head_size, num_heads, sequence_length):
        ...
        # TODO: Write your code here
        # ==========================
        self.q = nn.Linear(head_size*num_heads, head_size*num_heads, bias=True)
        self.v = nn.Linear(head_size*num_heads, head_size*num_heads, bias=True)
        self.k = nn.Linear(head_size*num_heads, head_size*num_heads, bias=True)
        self.out = nn.Linear(head_size*num_heads, head_size*num_heads)
    ...
    def get_attention_weights(self, queries, keys):
        ...
        return F.softmax((torch.matmul(queries, keys.transpose(-2, -1)) /  np.sqrt(len(keys[0][0][0]))).masked_fill(torch.tril(torch.ones((len(keys[0][0]), len(keys[0][0])))).expand(
            len(keys), 1, len(keys[0][0]), len(keys[0][0])) == 0, -1e9), dim=-1)
    ...
    def apply_attention(self, queries, keys, values):
        ...
        return torch.matmul(attention, values).transpose(1,2).reshape(len(queries),len(queries[0][0]),-1)
    ...
    def split_heads(self, tensor):
        ...
        return tensor.view(tensor.shape[0], -1, self.num_heads, int(tensor.shape[2]/self.num_heads)).transpose(1,2)
    ...
    def forward(self, hidden_states):
        ...
        Y= self.apply_attention(self.q(hidden_states).view(len(hidden_states), -1, self.num_heads, self.head_size).transpose(1,2),self.k(hidden_states).view(len(hidden_states), -1, self.num_heads, self.head_size) .transpose(1,2),self.v(hidden_states).view(len(hidden_states), -1, self.num_heads, self.head_size).transpose(1,2))
        return self.out(Y)
...
class MiniGPT1(nn.Module):
    ...
    def get_embeddings(self, inputs):
        ...
        # TODO: Write your code here
        # ==========================
        return self.embedding.forward(inputs,torch.arange(0, self.sequence_length).expand(len(inputs), self.sequence_length))
    ...
    def forward(self, inputs):
        ...
        # TODO: Write your code here
        # ==========================
        embedin=self.get_embeddings(inputs)
        for i,l in enumerate(self.layers):
            embedin = l(embedin)
        return F.log_softmax(self.classifier(embedin),dim=2)
    ...
    def loss(self, log_probas, targets, mask):
        ...
        for seq in range(len(targets)):
          ...
          s=sum(mask[seq])
          for n in range(int(s.item())):

              w=targets[seq][n]
              s1=s1+(log_probas[seq][n][w])

          batch_loss.append(s1/s*-1)
        mean=torch.FloatTensor(batch_loss).mean()
