...
class LSTM(nn.Module):
    ...
    def forward(self, inputs, hidden_states):
        ...
        #a=embedding of the input
        #b=lstm of 'a' and hidden states
        #c=classifier of 'b'
        #d=log softmax of 'c'
        #Return d
        
        a = self.embedding(inputs)
        b,hidden = self.lstm(a,hidden_states)
        preds = self.classifier(b)
        log_proba = F.log_softmax(preds,dim=2)
    ...
    def loss(self, log_probas, targets, mask):
        ...
        # TODO: Write your code here
        # ==========================
        #len_seq = torch.sum(mask,-1)
        #targets = torch.unsqueeze(targets, 2)
        #decode = torch.zeros_like(log_probas,)
        #targets = decode.scatter(2, targets, 1)
        #log_probas = targets*log_probas
        #log_probas = torch.sum(log_probas,-1)
        ...
        #return (-1*(torch.sum(log_probas,-1)/len_seq)).mean()
        #return loss
        batch_loss = []
        for i in range(len(targets)):
            ...
            x = 0
            s = sum(mask[i])
            for j in range(int(s.item())):
                           tar = targets[i][j]
                           los = los+log_probas[i][j][tar]
          
            batch_loss.append(los/s*-1)
        loss = torch.Tensor(batch_loss).mean()
